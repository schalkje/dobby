...
<!-- Generated by scripts/build-skills.py from skills/ado/dobby-create-pbi/SKILL.md — do not edit this copy; edit the source and regenerate. -->
...
9. **Use canonical `skills/` paths for all file reads and script invocations.** Reference scripts and templates from the canonical `skills/` directory — e.g., `python dobby-create-pbi/scripts/azdo-update-fields.py`, not from `.github/skills/` or `.claude/skills/` host copies.
...
The `az boards` CLI does not support multiline markdown fields — it truncates content at newlines and cannot set the field format to markdown. The helper script `dobby-create-pbi/scripts/azdo-update-fields.py` handles this via the REST API.

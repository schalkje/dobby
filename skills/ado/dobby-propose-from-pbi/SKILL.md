...
- **Use canonical `skills/` paths.** Reference scripts and templates from the canonical `skills/` directory — e.g., `python skills/_lib/azdo-update-fields.py`, not from `.github/skills/` or `.claude/skills/` host copies.
...
Use the helper script at `skills/_lib/azdo-update-fields.py` to update fields as Markdown:

...
description: Create an OpenSpec proposal from an Azure DevOps PBI. Fetches work item details and generates proposal, design, and task artifacts ready for implementation.
metadata:
  author: dobby
  version: "1.0"
---
...
These defaults eliminate repeated prompts for org and project. If the file does not exist, collect org and project interactively (same pattern as `dobby-create-pbi`).
...
**2a.** Load `.dobby/azdo-defaults.json` if it exists, merge with `az devops configure --list` (file takes priority).
...
Use the helper script at `skills/dobby-create-pbi/scripts/azdo-update-fields.py` to update fields as Markdown:

...
description: Create a PBI/issue in this project's tracker. Auto-detects Azure DevOps, GitHub, or Combined mode from .dobby/config.json and hands off to the matching backend skill. Use this for any "create a pbi", "create an issue", "make a work item", or "open a ticket" request.
metadata:
  author: dobby
  version: "3.0"
---

Create a work item in whichever tracker this project uses. This skill is a **dispatcher** — it reads the project's backend configuration and hands off to the right backend implementation. It does not talk to any tracker itself.
...
|-----------------|--------------------------------------------------|
| `"ado"`         | `skills/dobby-ado-create-pbi/SKILL.md`           |
| `"github"`      | `skills/dobby-gh-create-issue/SKILL.md`          |
| `"combined"`    | `skills/dobby-ado-create-pbi/SKILL.md` *(work items live in ADO)* |

**Combined mode note**: For `"combined"`, work item creation routes to ADO — the work item lives in Azure DevOps. The GitHub repo is used only for branches, PRs, and code. Before handing off, verify both identities:
```bash
...
- This skill NEVER creates a work item itself. Routing only.
